<?php
/**
 * new-demo functions and definitions
 *
...
if ( ! function_exists( 'new_demo_setup' ) ) :
	/**
	 ...
	 */ 
	function new_demo_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on new-demo, use a find and replace
		 * to change 'new-demo' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'new-demo', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'new-demo' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'new_demo_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'new_demo_setup' );
 ...
 */
function new_demo_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'new_demo_content_width', 640 );
}
add_action( 'after_setup_theme', 'new_demo_content_width', 0 );
 ...
 */
function new_demo_widgets_init() {
	register_sidebar(
		array(
...
add_action( 'widgets_init', 'new_demo_widgets_init' );
...
// function new_demo_scripts() {
// 	wp_enqueue_style( 'new-demo-style', get_stylesheet_uri(), array(), _S_VERSION );
// 	wp_style_add_data( 'new-demo-style', 'rtl', 'replace' );

// 	wp_enqueue_script( 'new-demo-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

// 	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
// 		wp_enqueue_script( 'comment-reply' );
// 	}
// } 
add_action( 'wp_enqueue_scripts', 'new_demo_scripts' );
